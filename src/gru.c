...
#define SIGMOID_SHIFT 11  /* For sigmoid LUT: input range [-8, 8] */
#define TANH_SHIFT 10     /* For tanh LUT: input range [-4, 4] */
#define BIAS_SCALE 127

void gru_init(GRU_Model *model, const uint8_t *weights_data) {
  /* Per-layer scales are loaded from file by model_load_distributed */
  /* For legacy blob loading, we just initialize hidden state */
...
void gru_forward(GRU_Model *model, uint8_t input_char, int32_t *output) {
  ...
  /* Output: W_out @ h + b_out (return as int32 for sampling) */
